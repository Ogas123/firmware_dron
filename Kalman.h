...
void updateKalmanAltura(float x[2], float P[2][2], float acc_z_g, float dist_tof_m, float roll_est_deg, float pitch_est_deg, bool nuevaMedicionToF);

// Firma actualizada
void actualizarFiltrosLQG(float u_roll, float u_pitch, float u_yaw, float u_alt,
                          float y_roll[2], float y_pitch[2], float y_yaw, 
                          float dist_tof_m, float acc_z_g, bool nuevaMedicionToF);
